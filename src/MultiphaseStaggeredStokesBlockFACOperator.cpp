        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            else // use constant xi coefficient
            {
                ...
                for (SAMRAI::pdat::CellIterator<NDIM> ci(patch->getBox()); ci; ci++) // cell-centers
                {
                    ...
                    // define ths variables here
                    // thn at corners 
                    double thn_imh_jph = 0.25 * ((*thn_data)(idx) + (*thn_data)(idx - xp) + (*thn_data)(idx + yp) +
                                                    (*thn_data)(idx - xp + yp)); // thn(i-1/2,j+1/2)
                    double thn_imh_jmh = 0.25 * ((*thn_data)(idx) + (*thn_data)(idx - xp) + (*thn_data)(idx - yp) +
                                                    (*thn_data)(idx - xp - yp)); // thn(i-1/2,j-1/2)
                    double thn_iph_jph = 0.25 * ((*thn_data)(idx) + (*thn_data)(idx + xp) + (*thn_data)(idx + yp) +
                                                    (*thn_data)(idx + xp + yp)); // thn(i+1/2,j+1/2)
                    double thn_iph_jmh = 0.25 * ((*thn_data)(idx) + (*thn_data)(idx + xp) + (*thn_data)(idx- yp) +
                                                    (*thn_data)(idx + xp - yp)); // thn(i+1/2,j-1/2)
                    // define ths variables here                
