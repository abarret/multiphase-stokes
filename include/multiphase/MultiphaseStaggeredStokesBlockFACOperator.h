...
public:
     ...
     * 
     * \note This method performs Gauss-seidel iterations on the Stokes block.
     */
...
private:
     ...
     */
    MultiphaseStaggeredStokesBlockFACOperator() = delete;
     ...
     */
    MultiphaseStaggeredStokesBlockFACOperator(const MultiphaseStaggeredStokesBlockFACOperator& from) =
        delete;
     ...
     */
    MultiphaseStaggeredStokesBlockFACOperator&
    operator=(const MultiphaseStaggeredStokesBlockFACOperator& that) = delete;

    /*!
     * \brief Perform prolongation or restriction on the provided indices.
     */
    //\{
    void performProlongation(const std::array<int, 2>& dst_idxs, const std::array<int, 2>& src_idxs, int dst_ln);
    void performRestriction(const std::array<int, 2>& dst_idxs, const std::array<int, 2>& src_idxs, int dst_ln);
    void performGhostFilling(const std::array<int, 2>& dst_idxs, int dst_ln);
    //\}
    ...
    SAMRAI::tbox::Pointer<SAMRAI::xfer::RefineOperator<NDIM>> d_un_prolong_op, d_us_prolong_op;
    SAMRAI::tbox::Pointer<SAMRAI::xfer::CoarsenOperator<NDIM>> d_un_restrict_op, d_us_restrict_op;
